...
class hourglass(nn.Module):
    ...
    def forward(self, x):
        ...
        return conv6
class OffsetY(nn.Module):
    def __init__(self, in_channels, hidden_channels=64):
        super().__init__()
        self.regressor = nn.Sequential(
            nn.Conv2d(in_channels, hidden_channels, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden_channels, 1, kernel_size=3, padding=1)  # 输出 y-offset，范围可以是 [-max, max]
        )

    def forward(self, offset_volume):
        y_offset = self.regressor(offset_volume)  # [B, 1, H, W]
        return y_offset
class cfnet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        ...
        self.vert_conv_gw = nn.Sequential(
            nn.Conv2d(160,160,(7,1),padding=(3,0)),
            nn.ReLU(),
            nn.Conv2d(160,160,(7,1),padding=(3,0)),
        )

        self.vert_conv_ca = nn.Sequential(
            nn.Conv2d(12,12,(7,1),padding=(3,0)),
            nn.ReLU(),
            nn.Conv2d(12,12,(7,1),padding=(3,0)),
        )
    ...
    def forward(self, left, right):
        ...
        #求dmin dmax  N = dmax - dmin + 1
        dmin = mindisparity_s3_1.min().item()
        dmax = maxdisparity_s3_1.max().item()
        dmin = math.floor(dmin)
        dmax = math.ceil(dmax)
        N = dmax - dmin + 1
        feature3_concat_right = self.vert_conv_ca(features_right["concat_feature3"]) # [B,12,H,W]
        t = build_concat_feature(features_left["concat_feature3"], feature3_concat_right, dmin, dmax)
        # t [B, (N+1)*12, H, W]
        regressor = OffsetY(in_channels=(N + 1) * 12).cuda()
        y_offset_s3 = regressor(t)  # [B, 1, H, W]。H = H /4

        #对左特征图进行warp 
        feature_left_concat3 = warp_left(features_left["concat_feature3"], y_offset_s3)
        feature_left_gw3 = warp_left(features_left["gw3"], y_offset_s3)

        y_offset_s2 = F.interpolate(y_offset_s3, scale_factor=2.0, mode="bilinear", align_corners=True) * 2.0

        feature_left_concat2 = warp_left(features_left["concat_feature2"], y_offset_s2)
        feature_left_gw2 = warp_left(features_left["gw2"], y_offset_s2)

        y_offset = F.interpolate(y_offset_s2, scale_factor=2.0, mode="bilinear", align_corners=True) * 2.0


        disparity_samples_s3 = self.generate_disparity_samples(mindisparity_s3_1, maxdisparity_s3_1, self.sample_count_s3).float()
        confidence_v_concat_s3, _ = self.cost_volume_generator(feature_left_concat3,
                                                            features_right["concat_feature3"], disparity_samples_s3, 'concat')
        confidence_v_gwc_s3, disparity_samples_s3 = self.cost_volume_generator(feature_left_gw3, features_right["gw3"],
        ...
        confidence_v_concat_s2, _ = self.cost_volume_generator(feature_left_concat2,
                                                            features_right["concat_feature2"], disparity_samples_s2, 'concat')
        confidence_v_gwc_s2, disparity_samples_s2 = self.cost_volume_generator(feature_left_gw2, features_right["gw2"],
        ...
        if self.training:
            ...
            return [pred0_4, pred1_4, pred2_s4, pred0_s3, predmid_s3, pred1_s3_up, pred0_s2, predmid_s2, pred1_s2], y_offset
